...
use App\Http\Requests\Product_Categoriescategory_addRequest;
use App\Http\Requests\Product_Categoriescategory_editRequest;
...
class Product_CategoriesController extends Controller
{
	...
	function category_add(){
		return $this->renderView("pages.product_categories.category_add");
	}
     ...
     * @return \Illuminate\Http\Response
     */
	function category_add_store(Product_Categoriescategory_addRequest $request){
		$modeldata = $this->normalizeFormData($request->validated());
		
		//save Product_Categories record
		$record = Product_Categories::create($modeldata);
		$rec_id = $record->id;
		return $this->redirect("product_categories/categories_list", __('recordAddedSuccessfully'));
	}
     ...
     * @return \Illuminate\View\View
     */
	function categories_list(Request $request, $fieldname = null , $fieldvalue = null){
		$view = "pages.product_categories.categories_list";
		$query = Product_Categories::query();
		$limit = $request->limit ?? 10;
		if($request->search){
			$search = trim($request->search);
		...
		$records = $query->paginate($limit, Product_Categories::categoriesListFields());
		return $this->renderView($view, compact("records"));
	}
     ...
     * @return \Illuminate\View\View;
     */
	function category_edit(Product_Categoriescategory_editRequest $request, $rec_id = null){
		$query = Product_Categories::query();
		$record = $query->findOrFail($rec_id, Product_Categories::categoryEditFields());
		if ($request->isMethod('post')) {
			$modeldata = $this->normalizeFormData($request->validated());
			$record->update($modeldata);
			return $this->redirect("product_categories/categories_list", __('recordUpdatedSuccessfully'));
		}
		return $this->renderView("pages.product_categories.category_edit", ["data" => $record, "rec_id" => $rec_id]);
	}
}
