    ...
    <div class="master-detail-page card">
        <div class="card-header text-bold h5 p-3 mb-3">Source Documents Records</div>
        <div class="p-2">
            <ul class="nav nav-pills">
                <li class="nav-item">
                    <a data-bs-toggle="tab" href="#invoice_documents_<?php echo $page_id ?>" class="nav-link active">
                    {{ __('sourceDocumentsInvoiceDocuments') }}
                </a>
            </li>
        </ul>
    </div>
    <div class="tab-content">
        ...
        <div class=" ">
            <?php
                $params = ['doc_type' => $rec_id,'show_header' => false]; //new query param
                $query = array_merge(request()->query(), $params);
                $queryParams = http_build_query($query);
                $url = url("invoice_documents/index/doc_type/$rec_id?$queryParams");
            ?>
